...
type MySQLConfigurationSettings struct {
	...
	VitessCells              []string // Name of the Vitess cells for polling Vitess tablet hosts
	VitessRealtimeStats      bool     // Use realtime stats for polling Vitess tablet health
...
func (settings *MySQLConfigurationSettings) postReadAdjustments() error {
	...
	for _, clusterSettings := range settings.Clusters {
		...
		if !clusterSettings.VitessSettings.IsEmpty() {
			if len(clusterSettings.VitessSettings.Cells) < 1 {
				clusterSettings.VitessSettings.Cells = settings.VitessCells
			}
			if settings.VitessRealtimeStats {
				clusterSettings.VitessSettings.RealtimeStats = true
			}
		}
	}
	return nil
